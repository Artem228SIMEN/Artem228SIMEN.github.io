// Элемент, который обеспечивает группировку слоев.
...
L.Control.GroupedLayers = L.Control.extend({
  ...
  initialize: function (
    ...
    for (i in groupedOverlays2) {
      for (var j in groupedOverlays2[i]) {
        this._addLayer(groupedOverlays2[i][j], j, i, true);
      }
    }
  },
  ...
  _addItem: function (obj) {
    ...
    if (obj.overlay) {
      ...
      // Создаём контейнер группы, если его не было
      if (!groupContainer) {
...
L.control.groupedLayers = function (
  baseLayers,
  groupedOverlays,
  groupedOverlays2,
  options
) {
  return new L.Control.GroupedLayers(
    baseLayers,
    groupedOverlays,
    groupedOverlays2,
    options
